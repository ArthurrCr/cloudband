...
def test_reference_requires_a_dataset() -> None:
    with pytest.raises(ValueError, match="must name the dataset"):
        phase0.Reference(name="r", model_id="m", dataset_id="", boa={"cloud": 90.0})


def test_reference_requires_a_model() -> None:
    with pytest.raises(ValueError, match="must name the model"):
        phase0.Reference(name="r", model_id="", dataset_id="d", boa={"cloud": 90.0})


def test_reference_rejects_unknown_experiments() -> None:
    with pytest.raises(ValueError, match="unknown experiments"):
        phase0.Reference(name="r", model_id="m", dataset_id="d", boa={"haze": 90.0})


def test_published_references_name_model_and_dataset() -> None:
    published = (
    ...
    assert all(reference.dataset_id and reference.model_id for reference in published)
    assert phase0.STUPMASK_CLOUDSEN12.model_id == phase0.STUPMASK_MODEL
    assert set(phase0.STUPMASK_CLOUDSEN12.boa) == {"cloud"}


def test_matching_model_and_dataset_is_a_reproduction_check(monkeypatch) -> None:
    ...
    assert "reproduction check" in phase0.describe_comparison(frame)


def test_different_model_same_dataset_is_flagged(monkeypatch) -> None:
    """The STUPmask value is another model, not a reproduction target."""
    samples = [dataset.Sample("scene_0", band_stack([1, 1, 1, 1]), annotation([0, 1, 2, 3]))]
    monkeypatch.setattr(dataset, "iter_samples", fake_iterator(samples))
    ...
    assert not bool(frame["same_model"].any())
    description = phase0.describe_comparison(frame)
    assert "different model" in description
    assert "not reproduction fidelity" in description


def test_different_dataset_is_flagged(monkeypatch) -> None:
    samples = [dataset.Sample("scene_0", band_stack([1, 1, 1, 1]), annotation([0, 1, 2, 3]))]
    monkeypatch.setattr(dataset, "iter_samples", fake_iterator(samples))

    result = phase0.run(make_table(1), constant_predictor([0, 1, 1, 3]), phase0.OCM_CURRENT_MODEL)
    frame = phase0.compare_to_reference(result, phase0.PIXBOX_S2_V1_7_0)
    assert bool(frame["same_model"].all())
    assert not bool(frame["same_dataset"].any())
    assert "populations differ" in phase0.describe_comparison(frame)
