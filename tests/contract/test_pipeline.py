...
def test_loader_names_differ_per_sensor() -> None:
    from cloudband.baselines import ocm

    assert ocm.SENTINEL2 != ocm.LANDSAT8
    assert {ocm.SENTINEL2, ocm.LANDSAT8} == {"sentinel2", "landsat8"}


def test_unknown_sensor_is_rejected() -> None:
    """The check runs before the package import, so it works without it."""
    from cloudband.baselines import ocm

    with pytest.raises((ValueError, ModuleNotFoundError)):
        ocm.load_func("himawari")